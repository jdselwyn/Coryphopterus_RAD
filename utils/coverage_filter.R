...
mean_depth <- read_lines('../tmp_dir/MiSeq_CHYA_chyaH.2.1.Fltr04.18.site.depth.mean') %>%
  extract(-1) %>%
  as.numeric()
...
out %>%
  arrange(-m) %>%
  dplyr::select(-n_less, -statistic:-parameter, -method:-alternative) %>%
  mutate(across(c(estimate, starts_with('conf')), ~ .*593)) %>%
