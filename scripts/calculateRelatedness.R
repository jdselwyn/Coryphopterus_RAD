...
#### Testing ####
## How Long
# start <- end <- time_difference <- vector('list', 10)
# for(i in 1:10){
#   start[[i]] <- Sys.time()
  # both_ind <- sample(unique(c(all_pairs$sample1, all_pairs$sample2)), 2)
#   calculate_relatedness_pair(ind1 = both_ind[1],
#                              ind2 = both_ind[2], 
#                              gds_file = gds_file,
#                              rel_method = 'EM', N_unrel = 1000, N_boot = 1000)
#   end[[i]] <- Sys.time()
#   time_difference[[i]] <- end[[i]] - start[[i]]
# }
# 
# map_dbl(time_difference, as.numeric) %>%
#   range %>%
#   map_dbl(~.x %>%
#         multiply_by(nrow(vcf_summary$individuals[[1]]) * (nrow(vcf_summary$individuals[[1]]) - 1) / 2) %>%
#           # multiply_by(1000) %>%
#         divide_by(40) %>% #Number of processors - assuming linear
#         divide_by(60) %>% #seconds to minutes
#         divide_by(60)) #minutes to hours
# #~16 - 36 hours

#### Calculate Relatedness ####
# relatedness2 <- all_pairs %>%
#   mutate(out = future_map2(sample1, sample2, calculate_relatedness_pair,
#                          gds_file = gds_file,
#                          N_unrel = UNREL,
#                          N_boot = BOOT,
#                          rel_method = 'EM',
...
#   unnest(out) #%T>%
#   write_csv(str_replace(gds_file, '\\.gds$', '_relatedness.csv'))


# calculate_relatedness_pair(sample(all_pairs$sample1, 1), 
#                                   sample(all_pairs$sample2, 1), 
#                            gds_file = gds_file,
#                            N_unrel = UNREL,
#                            N_boot = BOOT,
#                            rel_method = 'EM')
...
# relatedness <- run_relatedness(all_pairs)
# readr::write_csv(relatedness, 
#                  stringr::str_replace(gds_file, '\\.gds$', '_relatedness.csv'))

relatedness <- character(SUBGROUPS)
for(i in 1:SUBGROUPS){
  message('Start subset ', i, ': ', Sys.time())
  
  tmp_rel <- run_relatedness(all_pairs[[i]])
  
  relatedness[i] <- stringr::str_replace(gds_file, '\\.gds$', paste0('_relatednessSubset_', i,'.csv'))
  readr::write_csv(tmp_rel, relatedness[i])
  rm(tmp_rel)
  gc(verbose = FALSE)
  
  message('Finished subset ', i, ': ', Sys.time())
}

relatedness <- tibble::tibble(file = relatedness) %>%
  dplyr::rowwise() %>%
  dplyr::summarise(readr::read_csv(file,
                                   col_types = readr::cols(k0 = readr::col_double(),
                                                    k1 = readr::col_double(),
                                                    loglik = readr::col_double(),
                                                    niter = readr::col_double(),
                                                    number_loci = readr::col_double(),
                                                    unrel = readr::col_character(),
                                                    boot_rel = readr::col_character(),
                                                    ind1 = readr::col_character(),
                                                    ind2 = readr::col_character())), 
                   .groups = 'drop') 
